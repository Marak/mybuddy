...
downloader.clone = function (cb) {
  ...
  git.clone({ 
    ...
    console.log('About to install App Storelocally to: ' + buddyPondAppStorePath);
    console.log('Contacting Github...');
    console.log('Attempting to clone Buddy Pond from: https://github.com/marak/buddypond-appstore');
    git.clone({ 
      fs,
      http,
      dir: buddyPondAppStorePath,
      url: 'https://github.com/marak/buddypond-appstore',
      onMessage: console.log
    }).then(function(){
      console.log('App Store has been downloaded to: ' + buddyPondAppStorePath)
      cb(null);
    });
  });
}
